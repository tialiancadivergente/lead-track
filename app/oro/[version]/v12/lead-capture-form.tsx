...
import { leadCaptureFormSchema, type LeadCaptureFormValues } from "@/app/modules/lead-capture/lead-capture.model";
...
function formatPhoneByDdi(value: string, ddi: string): string {
  ...
  if (numericValue.length > 3 && numericValue.length <= 7) return `${numericValue.slice(0, 3)}-${numericValue.slice(3)}`;
  if (numericValue.length > 7) return `${numericValue.slice(0, 3)}-${numericValue.slice(3, 7)}-${numericValue.slice(7)}`;
...
export function LeadCaptureForm({
  ...
  const {
    ...
    defaultValues: { email: "", ddi: "+55", whatsapp: "", ...defaultValues },
  });

  const ddi = watch("ddi");

  const submitHandler: SubmitHandler<LeadCaptureFormValues> = async (values) => {
    await onSubmit({ ...values, normalizedPhone: normalizePhone(values) });
  };

  return (
    <form id={formId} name={formName ?? ""} className={cn("w-full space-y-4", className)} onSubmit={handleSubmit(submitHandler)}>
      <div>
        <Input id="form-field-email" type="email" placeholder="Seu melhor e-mail" className={emailInputClassName} style={{ fontFamily: "Arial, sans-serif" }} {...register("email")} />
      ...
      <div className="relative">
        ...
        <div className="flex">
          <select className={ddiSelectClassName} style={{ fontFamily: "Arial, sans-serif" }} {...register("ddi")}>
        ...
        {errors.ddi && <p className="mt-1 text-sm !text-red-300">{errors.ddi.message}</p>}
        {errors.whatsapp && <p className="mt-1 text-sm !text-red-300">{errors.whatsapp.message}</p>}
      </div>
